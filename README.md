...
После запуска с API можно будет работать по адресу `localhost:8080`. Фронтенд будет запущен по адресу `localhost:5500`
...
|`GET`|`/admin/users`|Получить список всех пользователей|-|`ID` - ID пользователя, `Email` - email пользователя, `IsAdmin` - является ли пользователь админом|
